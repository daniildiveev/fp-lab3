...
spec = describe "Linear interpolation" $ do
  ...
  it "safeInterval is the segment between two points" $
    linearSafeInterval LinearConfig (Window [Point 1 10, Point 3 30]) `shouldBe` Interval 1 3

  it "evaluates exactly on a line y = a*x + b" $ do
    let a = 2
        b = 1
        w = Window [Point 0 (a * 0 + b), Point 10 (a * 10 + b)]
        yMid = linearEvalAt LinearConfig w 5
    yMid `shouldSatisfy` approx (a * 5 + b)
    ...
    property $ \(Finite a) (Finite b) ->
      let x1 = 1.0 :: Double
       ...
       in forAll (choose (x1, x2)) $ \x ->
            approx (linearEvalAt LinearConfig w x) (a * x + b)
